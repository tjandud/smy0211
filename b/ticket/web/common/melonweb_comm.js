  ...
  "use strict"; 
    ...
    */
    ...
    WEBSVC.define('WEBSVC.Cookie', /** @lends MELON.WEBSVC.Cookie */{
         ...
         */
        getMLCPHeader: (function(){
            var keyNames = 'memberKey,memberId,memberDjYn,memberArtistId,token,facebookConnect,?,displayId,memberType,displayLoginId,cookieLoginType'.split(',');

            return function(Name) {
                var strBuf = unescape(decodeURI(Base64.decode(MELON.WEBSVC.Cookie.get('MLCP'))));
                if (strBuf === null) { return null; }
      ...
      * 회원정보 API 호출
      */
    WEBSVC.define('WEBSVC.MemberAPI', /** @lends MELON.WEBSVC.MemberAPI */{

         // getApiMemberProperties: function(method) {
         //     var result = null;
         ...
         //     return result;
         // }
         getApiMemberProperties: (function(){
    ...
    WEBSVC.define('PBPGN.ToggleSlider', function() {
        ...
        var ToggleSlider = Class(/** @lends MELON.PBPGN.ToggleSlider# */{
            ...
            initialize: function(el, options) {
                ...
                me.nowpage = 0;
                me.maxpage = me.$contents.length - 1;
...
(function (context, $, WEBSVC, PBPGN, undefined) {
    ...
    WEBSVC.define('WEBSVC.Auth', /** @lends MELON.WEBSVC.Auth */{
        ...
        isMelonLogin: function() {
            var memberKey = this.getMemberKey();
        ...
        getMemberKey: function() {
            return Cookie.getMLCPHeader("memberKey");
        },
        ...
        getMemberNickName: function() {
            var nickName = this.getDisplayId();
            if(!nickName){
                return "닉네임 없음";
            }
            return nickName;
        ...
        getDisplayId: function () {
            return Cookie.getMLCPHeader("displayId");
        },
        ...
        getMemberAge: function() {
            return MemberAPI.getApiMemberProperties("getMemberAge");
        },
        ...
        getMemberSex: function() {
            return MemberAPI.getApiMemberProperties("getMemberSex");
        },
        ...
        isMemberAdult: function() {
            var memberAge = this.getMemberAge();
            var realNameYn = this.isRealNameFlag();
            if (memberAge < 19) {
                return "0";
        ...
        isRealNameFlag: function() {
            return MemberAPI.getApiMemberProperties("getRealNameYn");
        },
        ...
        var MelonGNB = Class(/**@lends MELON.PBPGN.GNB.MelonGNB# */{
            ...
            _bindGNBMenu: function() {
                ...
                var $gnbMenu = $('#gnb_menu > ul:eq(0)'),
                    ...
                    moreLay2 = $gnbMenu.find('ul.gnr_more');
                var timerID = null;
        ...
        var PersonalArea = Class(/** @lends MELON.PBPGN.Personal.PersonalArea# */{
            ...
            _bindExpandToggle: function() {//140331_버튼클릭시에만 캐쉬에 모드가 저장되도록 수정
                ...
                me.$el.on('click', 'button.d_expand, button.d_collapse', function(e){
                    ...
                    me.options.isSaveCookie && WEBSVC.Cookie.set('personalexpand', isExpand ? 'true' : 'false');
...
window.addEventListener("message", accountAction, false);
function accountAction(orgMsg) {
    var accountMsgData = orgMsg.data;

    if (accountMsgData.messageFrom === "account") {
        if (accountMsgData.action === "refresh") {
            location.reload(true);
        }

        if (accountMsgData.action === "move" && accountMsgData.url !== undefined) {
            location.replace(decodeURIComponent(accountMsgData.url));
        }
    }
}